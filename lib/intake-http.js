...
function createRetryHandler(overrides) {
  return async function retry(req, res) {
    try {
      ...
      // Queue attention still proves a completed cycle; failures above do not.
      // This optional RPC is absent from previews and remains off until its schema is provisioned.
      if (deps.config.heartbeatEnabled && await deps.store.completedCycle() !== true) throw new IntakeError('storage_unavailable', 503);
